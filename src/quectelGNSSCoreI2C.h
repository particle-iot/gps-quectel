...
public:
     ...
     * @brief Transmit data to GNSS module
     *
     * @param pBuffer (in) pointer to data being sent to module
     * @param length (in) number of bytes being transmitted to module
     * 
     * @return true Transmit data success
     */
    bool write_data(uint8_t *pBuffer,uint32_t length);

    /**
     * @brief Receive GNSS data
     *
     * @param pBuffer (out) pointer to buffer containing data returned by the read request
     * @param length (in) size for pBuffer
     * 
     * @return true Receive data success
     */ 
    bool read_data(uint8_t *pBuffer,uint32_t length);

    /**
     * @brief Check the available length for reading
     *
     * @return The available length
     */
    uint32_t avaiable_for_read();

    /**
     * @brief Check the available length for writing
     *
     * @return The available length
     */
    uint32_t avaiable_for_write();
...
private:
    ...
    /**
     * @brief Check the available length of the GNSS specified buffer
     *
     * @param regOffset The register address
     *
     * @return The available length
     */
    uint32_t check_buffer_length(uint16_t regOffset);

    /**
     * @brief Convert the request packet to the byte array
     *
     * @param pRequests The request packet
     * @param pBuffer The request packet data buffer
     * @return none
     */
    void inline generate_request_packet(uint32_t *pRequests,uint8_t *pBuffer);

    /**
     * @brief Wrapper of Wiring API to read I2C bus
     *
     * @param addr 7-bit I2C device address to request data from
     * @param data (out) Buffer for returning received bytes
     * @param length (in) number of bytes to request from the module
     *
     * @return Error if data could not be received. Note that partial data may be returned
     */
    I2c_Resp_FlagStatus i2c_master_receive(uint8_t addr, uint8_t *data, uint32_t length);
     ...
     * @return Error if the full length could not be sent
     */  
    I2c_Resp_FlagStatus i2c_master_transmit(uint8_t addr, uint8_t *data, uint32_t length);
};
